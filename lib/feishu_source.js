 ...
 *
 * 数据来源（用户可配置）：
 *   默认指向用户配置的飞书 Base，可在 config/external_source.json 改成任何飞书 Base
 *
 * 持续更新策略：
...
function fetchPage(baseToken, tableId, offset = 0, limit = 200) {
  ...
  let stdout;
  // Windows 兼容（v4.9）：
  //   1) npm 全局命令是 .cmd 批处理包装器，execFileSync 无法直接执行 → 经 cmd.exe /d /s /c 代执行
  //   2) 服务器进程的 PATH 可能不含 npm 全局 bin → 把 node.exe 所在目录与 %APPDATA%\npm 补进子进程 PATH 兜底
...
function syncExternalCompanies(options = {}) {
  ...
  // 按开放日期排序，同日期内随机打乱（避免与原始表格顺序完全一致）
  allRows.sort((a, b) => (a.open_date || '').localeCompare(b.open_date || ''));
  const groups = {};
  for (const row of allRows) {
    const key = row.open_date || '__none';
