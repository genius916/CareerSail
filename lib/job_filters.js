...
// ============================================================
// 5. 岗位方向过滤（v4.5 开源通用化：完全配置驱动）
// 三级判定，关键词全部来自用户配置（config/user_profile.json），不硬编码任何专业偏好：
//   role_strong_exclude: 强剔除关键词 —— 命中即剔除（最高优先级），适合放"工程师""设计师"这类强信号
//   role_keep:           保留关键词 —— 命中即保留（如"产品""运营""管培"）
//   role_exclude:        剔除关键词 —— 未命中前两级时才生效（如"算法""研发""开发"）
// 未配置任何列表 → 不做方向过滤（适配所有专业：计算机用户可反向配置保留"研发/算法"）
// ============================================================

/**
 * 判断岗位是否为用户配置中排除的方向
 * @param {string} title - 岗位标题
 * @param {string} [category] - 岗位分类（可选）
 * @param {Object} [options] - { strongExclude: string[], keep: string[], exclude: string[] }
 * @returns {boolean} true = 不适合（应剔除）
 */
function isUnsuitableRole(title, category = '', options = {}) {
  const { strongExclude = [], keep = [], exclude = [] } = options;
  const text = `${title || ''} ${category || ''}`;
  if (!text.trim()) return false;
  if (strongExclude.length === 0 && keep.length === 0 && exclude.length === 0) return false;
  const hit = (arr) => Array.isArray(arr) && arr.some(kw => kw && text.includes(kw));
  // 判定顺序：强剔除 > 保留 > 一般剔除。
  // 这样"研发项目管理"命中保留词"项目管理"而保留，"产品工程师"命中强剔除词"工程师"而剔除。
  if (hit(strongExclude)) return true;
  if (hit(keep)) return false;
  if (hit(exclude)) return true;
  return false;
}

/**
 * 判断是否为真实岗位（过滤适配器降级产生的占位记录）
 * 占位特征：无 JD 链接、标题形如"XX公司 校园招聘"、备注含 Agent 验证提示
 * @param {Object} job
 * @returns {boolean} true = 真实岗位
 */
function isRealJob(job) {
  ...
  const note = job.note || '';
  if (!url) return false;
  if (/校园招聘(\（.*\）)?$/.test(title.trim()) && !/\d{3,}/.test(title)) return false;
  if (note.includes('Agent') || note.includes('待验证') || note.includes('浏览器验证')) return false;
  return true;
}
