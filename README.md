...
1. **同步秋招公司库**（飞书 Base，需 lark-cli）
2. **发现新开秋招的公司** —— 从公司库里找出「岗位池还没有的 + 近 7 天没搜过的」，按开招时间最新优先
3. **逐家搜索这些新公司的官网**，拉取真实岗位（带 JD 链接）
4. **按你的画像过滤后导入** —— 不适合你方向的岗位自动拦下，不进岗位池

已搜过的公司不会重复搜（短期不会出新岗位），所以每次刷新聚焦的都是「新开秋招」的公司。

### 个性化岗位过滤（可选，所有专业通用）

在 `config/user_profile.json` 里可以配置导入过滤，适配任何专业背景：

```json
{
  "role_strong_exclude": ["工程师", "设计师"],
  "role_keep": ["产品", "运营", "管培", "供应链"],
  "role_exclude": ["算法", "研发"],
  "excluded_companies": ["不想投的公司名"],
  "company_city_rules": [{ "company": "某公司", "cities": ["上海", "杭州"] }]
}
```

- `role_strong_exclude` 命中即剔除（优先级最高），适合放强信号词
- `role_keep` 命中即保留；`role_exclude` 在前两级未命中时才剔除
- **不配置就不做方向过滤** —— 计算机专业用户可以反过来把"研发/算法"放进 `role_keep`
