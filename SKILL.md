...
看板「🔄 刷新」面板里的「🧭 生成公司发现计划」按钮会调用 `/api/discover-plan`，从画像自动派生四维度关键词簇。Agent 按此计划逐层发现公司。
...
- 不凭空捏造公司名，所有公司必须来自搜索/官网真实结果
- 不硬编码"某专业对应某公司清单"，公司由画像派生的关键词簇驱动
...
看板的「🔄 刷新」按钮会：
1. 调用 `/api/search-targets` 获取当前搜索目标
2. 显示搜索目标面板，告诉 Agent 需要搜索哪些公司
3. Agent 根据 SKILL.md 中的标准流程执行搜索
4. 搜索完成后通过 `/api/import-batch` 批量写入岗位

**注意**：刷新按钮本身不执行搜索（因为搜索需要浏览器自动化能力，由Agent驱动）。刷新按钮的作用是：
- 重新加载CSV数据（即时生效）
- 生成搜索目标清单，提示Agent下一步该搜什么

Agent应该在用户点击刷新后，自动按照搜索目标执行搜索，并将结果写回CSV。
