...
7. **严禁广告链接（铁律）**：岗位链接绝不使用第三方广告/聚合站（如 chaojijianli.com、offcn.com、163.com、toutiao.com 等转载页）。链接优先级：**官方 JD 详情页 > 公司校招官网列表页/首页 > 如实告知用户"没找到链接"**。绝不为了填满链接字段而降级到广告站——宁可留官网首页或坦白说没有，也不给广告链接
8. **"校招实习转正"≠只招实习（铁律）**：岗位名称含"实习转正/校招实习"（如"品牌经理-校招实习转正"）是公司把校招与实习合并招聘，应届校招生可正常投递。不得仅因名称含"实习"就排除此类岗位，需进入 JD 描述确认招聘对象后再判定
...
- **例外——校招实习转正岗**：名称为"XX-校招实习转正/校招实习"类岗位（如元气森林"品牌经理-校招实习转正"）是校招与实习合并招聘，应届校招可投，**不要**按实习排除，进入 JD 确认后正常导入
...
> ⛔ **兜底线**：兜底只到公司校招官网为止。**任何情况下都不允许用第三方广告/聚合站链接（chaojijianli.com、offcn.com 等）填充 job_url**。官网链接也拿不到时，如实告知用户"该岗位未找到可用的官方链接"，由用户自行到官网/公众号查找
