...
- **飞书同步 Windows 兼容**（v4.9）：修复 `spawnSync lark-cli ENOENT` —— Windows 下 npm 全局命令是 `.cmd` 批处理包装器，`execFileSync` 无法直接执行，改为经 `cmd.exe /d /s /c` 代执行；同时把 node.exe 所在目录与 `%APPDATA%\npm` 补进子进程 PATH，服务器进程 PATH 缺失也能找到 lark-cli（实测同步 523 条恢复）。
